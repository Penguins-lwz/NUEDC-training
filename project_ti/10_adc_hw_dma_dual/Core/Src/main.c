...
myADC_HandleTypeDef myADC = {
	.htim = TIM_ADC_INST,
	.hadc0 = ADC0_INST, .adcch0 = ADC0_ADCMEM_CH0, .dmach0 = DMA_CH0_CHAN_ID,
	.hadc1 = ADC1_INST, .adcch1 = ADC1_ADCMEM_CH1, .dmach1 = DMA_CH1_CHAN_ID
};
...
void myADC_ConvCpltCallback(myADC_HandleTypeDef *myhadc)
{
	for (uint16_t i = 0; i < ADC_DataSize; ++i)
		myUART_Transmit(&myUART, "ADC_%d: %d, %d\n", i, myADC.pData0[i], myADC.pData1[i]);
}
...
/* 虽然使用两个 ADC，但因二者同步只需开启其中一个 ADC 的 DMA_DONE 中断 */
