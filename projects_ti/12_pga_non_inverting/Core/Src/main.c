...
void Key_Process(uint8_t keyNum)
{
	 ...
	 * S2 调整 OPA 增益，短按 1x，长按 2x
	 */
	if (keyNum == 0x01) LED_R_On, myADC_Start_DMA(&myADC);
	else if (keyNum == 0x02) LED_B_On, myOPA_Config(OPA0_INST, DL_OPA_GAIN_N0_P1);
	else if (keyNum == 0x81) LED_R_Off;
	else if (keyNum == 0x82) LED_B_Off, myOPA_Config(OPA0_INST, DL_OPA_GAIN_N1_P2);
}
